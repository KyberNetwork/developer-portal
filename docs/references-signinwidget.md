...
### Step 1: Configure Sign-In Widget Parameters
...
#### Example
`https://kyberswap.com/oauth/authorize?client_id=MYAPP123&redirect_uri=https://example.com/callback&response_type=code&state=CUSTOM_TOKEN`

### Step 2: Add Widget Into A Website
...
### Step 5: Obtain User Information
...
### Step 2: Call `/authorized_users` endpoint
`ACCESS_TOKEN` can be used to call Kyber's APIs using 1 of the following 2 methods:
1. Add request header authorization: Bearer `ACCESS_TOKEN` (recommended)
2. Include `access_token=ACCESS_TOKEN` as a GET or POST parameter
