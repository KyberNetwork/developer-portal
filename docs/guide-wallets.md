...
allowanceAmount = srcTokenContract.methods
  .allowance(USER_WALLET_ADDRESS, KYBER_NETWORK_PROXY_ADDRESS)
  .call();
...
transactionData = srcTokenContract.methods
  .approve(KYBER_NETWORK_PROXY_ADDRESS, srcTokenWeiPrice)
  .encodeABI();
...
var kyberNetworkProxyContractAddress = await provider
...
For convenience, a Web3 instance _can_ be injected by passing a web3 flag when requesting a provider:
